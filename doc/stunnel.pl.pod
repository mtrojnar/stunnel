...
=item Trybie zdalnym (opcja B<connect>) w systemie B<Linux E<gt>=2.6.28>
...
Konfiguracja ta wymaga, aby B<stunnel> by³ wykonywany jako root i bez opcji B<setuid>.

=item Trybie zdalnym (opcja B<connect>) w systemie B<Linux 2.2.x>
...
Dodatkowo B<stunnel> powinien byæ wykonywany jako root i bez opcji B<setuid>.

=item Trybie zdalnym (opcja B<connect>) w systemie B<FreeBSD E<gt>=8.0>

Konfiguracja ta wymaga skonfigurowania firewalla i routingu.
B<stunnel> musi byæ wykonywany jako root i bez opcji B<setuid>.

=item Trybie lokalnym (opcja B<exec>)

Konfiguracja ta jest realizowana przy pomocy biblioteki B<libstunnel.so>.
...
Oryginalny adres docelowy jest u¿ywany zamiast opcji B<connect>.
...
Opcja zosta³a przemianowana na B<source>.

=item B<no>

Opcja zosta³a przemianowana na B<none>.
...
=item B<poziom 1> - weryfikuj, je¿eli zosta³ przedstawiony

=item B<poziom 2> - weryfikuj z zainstalowanym certyfikatem Centrum Certyfikacji

=item B<poziom 3> - weryfikuj z lokalnie zainstalowanym certyfikatem drugiej strony

=item B<domy¶lnie> - nie weryfikuj
