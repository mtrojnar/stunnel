...
lub I<tcpserver>), nale¿y przeczytaæ sekcjê I<TRYB INETD> poni¿ej.
...
Aby nas³uchiwaæ na wszystkich adresach IPv6 nale¿y u¿yæ:
...
Funkcja skrótu zosta³a zmieniona w wersji 1.0.0 biblioteki OpenSSL.
Nale¿y wykonaæ c_rehash przy zmianie OpenSSL 0.x.x na 1.x.x.
...
posiadaæ specjalne nazwy XXXXXXXX.r0, gdzie XXXXXXXX jest skrótem
listy CRL.
...
Listê dostêpnych krzywych mo¿na uzyskaæ poleceniem:
...
U¿yj us³ugi jako podrzêdnej (virtualnego serwera) dla rozszerzenia TLS Server
Name Indication (RFC 3546).

I<nazwa_us³ugi> wskazuje us³ugê nadrzêdn±, która odbiera po³±czenia od klientów
przy pomocy opcji I<accept>.  I<nazwa_serwera> wskazuje nazwê serwera
wirtualnego.  Z pojedyñcz± us³ug± nadrzêdn± powi±zane jest zwykle wiele us³ug
podrzêdnych.  Opcja I<sni> mo¿e byæ rownie¿ u¿yta wielokrotnie w ramach jednej
us³ugi podrzêdnej.

Zarówno us³uga nadrzêdna jak i podrzêdna nie mo¿e byæ skonfigurowana w trybie
klienckim.  Opcja I<connect> us³ugi podrzêdnej jest ignorowana w po³±czeniu z
...
libwrap jest realizowane dwukrotnie: najpierw dla us³ugi nadrzêdnej po
odebraniu po³±czenia TCP, a nastêpnie dla us³ugi podrzêdnej podczas negocjacji
TLS.

Opcja I<sni> jest dostêpna pocz±wszy od wersji 1.0.0 biblioteki OpenSSL.
...
Opcji I<protocol> nie nale¿y u¿ywaæ z szyfrowaniem SSL na osobnym porcie.
...
Ten protokó³ jest wspierany wy³±cznie w trybie klienckim.
...
Zablokuj wsparcie dla przezroczystago proxy.  Jest to warto¶æ domy¶lna.
...
Konfiguracja wymaga nastêpuj±cych ustawieñ iptables oraz routingu
...
Dodatkowo B<stunnel> powinien byæ wykonywany jako root i bez opcji I<setuid>.
...
B<stunnel> musi byæ wykonywany jako root i bez opcji I<setuid>.
...
Konfiguracja wymaga nastêpuj±cych ustawieñ iptables
(na przyk³ad w pliku /etc/rc.local lub analogicznym):
...
U¿yj przezroczystego proxy zarówno dla adresu ¼ród³owego jak i docelowego.
...
Alternatywnie parametry DH mo¿na umie¶ciæ w pliku razem z certyfikatem:

    openssl dhparam 2048 >> stunnel.pem

Wygenerowanie parametrów DH mo¿e zaj±æ nawet wiele minut.
